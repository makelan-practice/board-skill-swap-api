...
public class MockDataStore
{
    ...
    private void SeedData()
    {
        ...
        // Пользователи: «Могу научить» — один навык (см. AddUser); «Хочу научиться» — несколько
        var u1 = AddUser("Иван", 34, 2, 2, new[] { sMusic.Id }, new[] { sTimeMgmt.Id, sYogaMeditation.Id, sSpanish.Id, sMindfulness.Id }, avatarUrl: "/Users/Ivan_34_spb.jpg", email: "ivan@mail.ru", password: "test", about: "Привет! Люблю ритм, кофе по утрам и людей, которые не боятся пробовать новое");   // СПб, Мужской
        var u2 = AddUser("Анна", 26, 5, 3, new[] { sEnglish.Id }, new[] { sYogaMeditation.Id, sNutrition.Id, sMindfulness.Id }, avatarUrl: "/Users/Anna_26_kaz.jpg", email: "anna@mail.ru", password: "test", about: "Работаю в стартапах, обожаю языки и новые проекты. Ищу баланс через практики и готова делиться опытом.");   // Казань, Женский
        var u3 = AddUser("Максим", 23, 1, 2, new[] { sPhoto.Id }, new[] { sSpanish.Id, sYogaMeditation.Id, sEnglish.Id, sFitness.Id }, avatarUrl: "/Users/Maksim_23_msk.jpg", email: "maksim@mail.ru", password: "test", about: "Фотограф и путешественник. Мечтаю свободно говорить по-испански и найти свой дзен через практику.");  // Москва, Мужской
        var u4 = AddUser("Елена", 30, 3, 3, new[] { sCooking.Id }, new[] { sEnglish.Id, sPhoto.Id, sPersonalDev.Id }, avatarUrl: "/Users/Elena_28_krasn.jpg", email: "elena@mail.ru", password: "test", about: "Готовлю с душой и практикую йогу. Хочу подтянуть английский для путешествий и новых знакомств.");  // Новосибирск, Женский
        var u5 = AddUser("Дмитрий", 28, 4, 2, new[] { sTimeMgmt.Id }, new[] { sMusic.Id, sDrawing.Id, sSleep.Id, sWorkLifeBalance.Id }, avatarUrl: "/Users/Dmitry_28_msk.jpg", email: "dmitry@mail.ru", password: "test", about: "Помогаю другим планировать время и достигать целей. Сам мечтаю наконец научиться играть на гитаре.");  // Екатеринбург, Мужской
        var u6 = AddUser("Ольга", 25, 1, 3, new[] { sYogaMeditation.Id }, new[] { sEntrepreneurship.Id, sPhoto.Id, sMarketing.Id, sEnglish.Id }, avatarUrl: "/Users/Olga_27_spb.jpg", email: "olga@mail.ru", password: "test", about: "Йога и осознанность — мой фундамент. Хочу развивать свой проект и научиться красиво его снимать.");  // Москва, Женский

        // Новые пользователи (аватары из wwwroot/Users)
        var u7 = AddUser("Кирилл", 24, 1, 2, new[] { sPhoto.Id }, new[] { sEnglish.Id, sVideoEdit.Id, sSales.Id }, avatarUrl: "/Users/Kirill_24_msk.jpg", email: "kirill@mail.ru", password: "test", about: "Снимаю людей и события. Хочу лучше планировать день и уверенно говорить по-английски.");           // Москва, Мужской
        var u8 = AddUser("Наталия", 27, 2, 3, new[] { sSpanish.Id }, new[] { sYogaMeditation.Id, sPhoto.Id, sEnglish.Id, sNutrition.Id, sMindfulness.Id }, avatarUrl: "/Users/Natalia_27_spb.jpg", email: "natalia@mail.ru", password: "test", about: "Учу испанский и люблю готовить. Хочу углубить практику йоги и научиться классно фотографировать.");   // СПб, Женский
        var u9 = AddUser("Григорий", 30, 5, 2, new[] { sEntrepreneurship.Id }, new[] { sEnglish.Id, sTimeMgmt.Id, sCoaching.Id }, avatarUrl: "/Users/Grigory_30_kaz.jpg", email: "grigory@mail.ru", password: "test", about: "Запускал свои проекты. Делюсь опытом и сам учусь эффективнее распоряжаться временем и английским.");   // Казань, Мужской
        var u10 = AddUser("Татьяна", 25, 4, 3, new[] { sYogaMeditation.Id }, new[] { sCooking.Id, sDrawing.Id }, avatarUrl: "/Users/Tatyana_25_ekb.jpg", email: "tatyana@mail.ru", password: "test", about: "ЗОЖ и йога — мой образ жизни. Мечтаю готовить так же вкусно и с любовью, как мама.");   // Екатеринбург, Женский
        var u11 = AddUser("Денис", 32, 3, 2, new[] { sMusic.Id }, new[] { sSpanish.Id, sGerman.Id, sVideoEdit.Id }, avatarUrl: "/Users/Denis_32_novosib.jpg", email: "denis@mail.ru", password: "test", about: "Играю в группе и веду проекты. Хочу выучить испанский для поездок в Латинскую Америку.");   // Новосибирск, Мужской
        var u12 = AddUser("Ирина", 23, 38, 3, new[] { sCooking.Id }, new[] { sPhoto.Id, sEnglish.Id, sVideoEdit.Id, sPersonalBrand.Id }, avatarUrl: "/Users/Irina_23_sochi.jpg", email: "irina@mail.ru", password: "test", about: "Готовлю с настроением, особенно выпечку. Хочу красиво снимать еду и свободно говорить по-английски.");   // Сочи, Женский
        var u13 = AddUser("Богдан", 28, 14, 2, new[] { sTimeMgmt.Id }, new[] { sMusic.Id, sJapanese.Id }, avatarUrl: "/Users/Bogdan_28_perm.jpg", email: "bogdan@mail.ru", password: "test", about: "Коуч и фанат тайм-менеджмента. Мечтаю научиться играть на инструменте — пока только слушатель.");   // Пермь, Мужской
        var u14 = AddUser("Станислав", 26, 22, 2, new[] { sEnglish.Id }, new[] { sEntrepreneurship.Id, sYogaMeditation.Id, sMarketing.Id, sResume.Id }, avatarUrl: "/Users/Stanislav_26_irk.jpg", email: "stanislav@mail.ru", password: "test", about: "Преподаю английский. Хочу свой бизнес и больше спокойствия — поэтому интересуюсь йогой.");   // Иркутск, Мужской
        var u15 = AddUser("Маргарита", 29, 24, 3, new[] { sDrawing.Id }, new[] { sCooking.Id, sNutrition.Id, sPhoto.Id, sMindfulness.Id }, avatarUrl: "/Users/Margarita_29_yar.jpg", email: "margarita@mail.ru", password: "test", about: "Рисую и делаю декор своими руками. Хочу готовить полезнее и разнообразнее.");   // Ярославль, Женский
        var u16 = AddUser("Тимофей", 31, 25, 2, new[] { sMusic.Id }, new[] { sTimeMgmt.Id, sJapanese.Id, sCoaching.Id, sVideoEdit.Id }, avatarUrl: "/Users/Timofey_31_vladiv.jpg", email: "timofey@mail.ru", password: "test", about: "Музыкант и фотограф с Дальнего Востока. Учу японский и ищу способы всё успевать.");   // Владивосток, Мужской
        var u17 = AddUser("Павел", 27, 29, 2, new[] { sVideoEdit.Id }, new[] { sPhoto.Id, sTimeMgmt.Id, sEnglish.Id, sFitness.Id }, avatarUrl: "/Users/Pavel_27_kem.jpg", email: "pavel@mail.ru", password: "test", about: "Монтаж и маркетинг — моя работа. Хочу круто снимать сам и лучше планировать день.");   // Кемерово, Мужской (11-й новый)
    ...
    /// <summary>Добавляет пользователя в мок-хранилище. Из массива teaching в «Могу научить» сохраняется только первый id.</summary>
    private User AddUser(string name, int age, int cityId, int genderId, int[] teaching, int[] learning, string? avatarUrl = null, string? email = null, string? password = null, DateOnly? dateOfBirth = null, string? about = null)
    {
        ...
        var u = new User
        {
            ...
            AvatarUrl = avatarUrl,
            About = about,
            TeachingSkillIds = teaching is { Length: > 0 } ? new List<int> { teaching[0] } : new List<int>(),
